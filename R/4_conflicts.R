#' Conflicts between the mapa and other packages
#'
...
#' @docType methods
#' @export
#' @return mapa conflicts
#' @examples
#' mapa_conflicts()
mapa_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", mapa_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "mapa_conflicts")
}

mapa_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "mapa_conflicts()"
  )
...
#' @export
print.mapa_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(mapa_conflict_message(x))
}
